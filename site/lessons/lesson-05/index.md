...
# Реструктуризация таблиц с `tidyr`

В анализе данных часто требуется менять форму таблиц, переводя их из **широкого** формата в **длинный** и наоборот. Для этого в пакете `tidyr` есть две мощные функции:

- **`pivot_longer()`** – перевод из **широкого** формата в **длинный**  
- **`pivot_wider()`** – перевод из **длинного** формата в **широкий**  

Давай разберем эти функции на примерах. 

---

### 📌 `pivot_longer()`: Преобразование в длинный формат

Допустим, у нас есть датафрейм с результатами измерений температуры в разных городах по месяцам:

```r
library(tidyverse)

temperature_data <- tibble(
  city = c("Москва", "Лондон", "Токио"),
  Jan = c(-5, 4, 6),
  Feb = c(-3, 5, 7),
  Mar = c(2, 9, 10)
)
...
Этот формат неудобен, если мы хотим, например, построить график температуры по месяцам. Нужно преобразовать его в **длинный формат**, где каждый месяц будет отдельной строкой.

📌 **Используем `pivot_longer()`**:
...
📊 **После преобразования (длинный формат):**
...
Теперь каждая строка соответствует одному городу в конкретный месяц с указанием температуры.

### 🛠 Как работает `pivot_longer()`?
- **`cols = Jan:Mar`** – указываем столбцы, которые превращаем в строки  
- **`names_to = "month"`** – создаем новую колонку `month`, которая будет содержать названия месяцев  
- **`values_to = "temperature"`** – создаем новую колонку `temperature`, куда будут записаны значения  

---

### 📌 `pivot_wider()`: Преобразование в широкий формат

Теперь представим, что у нас уже есть **длинный формат**, и мы хотим вернуть его обратно в **широкий формат**.

📌 **Используем `pivot_wider()`**:
...
📊 **После преобразования (широкий формат):**
...
### 🛠 Как работает `pivot_wider()`?
- **`names_from = month`** – создаем новые колонки на основе значений из `month`  
- **`values_from = temperature`** – заполняем их значениями из `temperature`  

---

## 🔥 Когда использовать `pivot_longer()` и `pivot_wider()`?

✅ **Используем `pivot_longer()`**, если:  
- Нужно перевести таблицу в более гибкий формат (удобный для анализа и графиков)  
- Данные представлены в нескольких колонках, но относятся к одной переменной  

✅ **Используем `pivot_wider()`**, если:  
- Нужно сделать таблицу более удобной для чтения  
- Нужно превратить строковые данные в отдельные колонки  

### 🚀 Итог
С помощью `pivot_longer()` и `pivot_wider()` можно легко менять форму данных и подстраивать таблицы под нужный анализ. Эти функции помогают делать данные более удобными для построения графиков, статистического анализа и машинного обучения. 🔥
